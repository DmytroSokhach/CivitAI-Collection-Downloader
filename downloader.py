...
def create_download_directory(collection_info):
    ...
    if not download_dir:
        ...
        logger.warning(
            f"Download directory not found in config, using default: {download_dir}"
        )
...
def ensure_download_url(url, base_name, api_key):
    """Ensure the URL is complete with domain and API key if needed."""
        ...
        try:
            ...
            with requests.get(url, stream=True) as response:
                ...
                if (
                    mime_type
                    and content_type
                    and not content_type.startswith(mime_type)
                ):
                    logger.warning(
                        f"MIME type mismatch. Expected: {mime_type}, Got: {content_type}"
                    )
            ...
            logger.error(
                f"Error downloading file (attempt {attempt+1}/{max_retries+1}): {e}"
            )
            if "response" in locals():
                logger.debug(
                    f"Response headers: {dict(response.headers) if hasattr(response, 'headers') else 'No headers'}"
                )
                logger.debug(
                    f"Response content: {response.text[:200] if hasattr(response, 'text') else 'No content'}"
                )
...
def download_media(media_data, download_dir, api_key=None):
    ...
    media_data_url_or_guid = media_data.get("url")
    mime_type = media_data.get("mimeType", "image/jpeg")

    if not media_data_url_or_guid:
    ...
    # Download the file
    url = ensure_download_url(media_data_url_or_guid, base_name, api_key)
    success = download_file(
        url,
        file_path,
        mime_type=mime_type if url == media_data_url_or_guid else None,
